...
class Image {
    ...
    public function __construct($image, $config = NULL)
    {
        ...
        if ($check === FALSE)
            throw new MException('image getimagesize missing');

        // Check to make sure the image exists
        if ( ! is_file($image))
            throw new MException('image file not found');
        ...
        if ( ! is_array($image_info) OR count($image_info) < 3)
            throw new MException('image file unreadable');

        // Check to make sure the image type is allowed
        if ( ! isset(Image::$allowed_types[$image_info[2]]))
            throw new MException('image type not allowed');
...
//        Yii::import("application.extensions.image.drivers.$driver");
        ...
        if ( ! ($this->driver instanceof Image_Driver))
            throw new MException('image driver must be implement Image_Driver class');
    }
    ...
    public function __get($property)
    {
        ...
        {
            throw new MException('invalid property');
        }
    }
    ...
    public function resize($width, $height, $master = NULL)
    {
        if ( ! $this->valid_size('width', $width))
            throw new MException('image invalid width');

        if ( ! $this->valid_size('height', $height))
            throw new MException('image invalid height');

        if (empty($width) AND empty($height))
            throw new MException('image invalid dimensions');
        ...
        elseif ( ! $this->valid_size('master', $master))
            throw new MException('image invalid master');
    ...
    {
        if ( ! $this->valid_size('width', $width))
            throw new MException('image invalid width', $width);

        if ( ! $this->valid_size('height', $height))
            throw new MException('image invalid height', $height);

        if ( ! $this->valid_size('top', $top))
            throw new MException('image invalid top', $top);

        if ( ! $this->valid_size('left', $left))
            throw new MException('image invalid left', $left);
    ...
    public function flip($direction)
    {
        if ($direction !== self::HORIZONTAL AND $direction !== self::VERTICAL)
            throw new MException('image invalid flip');
    ...
    public function save($new_image = FALSE, $chmod = 0644, $keep_actions = FALSE)
    {
        ...
        if ( ! is_writable($dir))
            throw new MException('image directory unwritable');
     ...
     * 
     * Output the image to the browser. 
     * @param boolean $is_direct
     * @param boolean $keep_actions
     * @return object 
     */
