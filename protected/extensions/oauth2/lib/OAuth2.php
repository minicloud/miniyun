...
* 免登陆
*/
define("OAUTH2_GRANT_TYPE_FREE_LOGIN", "free_login");
/**
 * Regex to filter out the grant type.
 */
define("OAUTH2_GRANT_TYPE_REGEXP", "/^(authorization_code|password|assertion|refresh_token|none|free_login)$/");
...
/**
 * "Verification Error" HTTP status code.
 *
 */
define("OAUTH2_HTTP_VERIFICATION", "402 Verification Error");
 ...
 * 
 */
define("OAUTH2_HTTP_DISABLED", "407 Disabled User");

/**
 * 
 */
define("OAUTH2_HTTP_LOCKED", "423 App Locked");
/**
 * @}
 */
...
define("OAUTH2_ERROR_INVALID_SIGN", "invalid_sign");
 ...
 * 用户被禁用
 */
define("SYSTEM_ERROR_USER_DISABLED", "user_disabled");

/**
 * 
 * app被禁用
 */
define("SYSTEM_ERROR_APP_DISABLED", "app_disabled");
/**
 * @}
 */
...
abstract class OAuth2 {
  ...
  abstract protected function getAccessToken($oauth_token);
     ...
     */
  abstract protected function setAccessToken($oauth_token, $client_id, $device_id, $expires, $scope = NULL);
  ...
  // So they're just stubs. Override the ones you need.
     ...
     *
     * @return array A list as below. If you support all grant types, then you'd do:@code
     * return array(
  ...
  protected function getSupportedGrantTypes() {
     ...
     *
     * @return array A list as below. If you support all authorization response types,@code
     * return array(
  ...
  protected function getSupportedAuthResponseTypes() {
     ...
     *
     * @return array A list as below, for example:@code
     * return array(
     *   'my-friends',
     *   'photos',
     *   'whatever-else',
  ...
  protected function checkRestrictedAuthResponseType($client_id, $response_type) {
     ...
     *
     * @return bool TRUE if the grant type is supported by this client identifier, and@ingroup oauth2_section_4
     */
  ...
  protected function checkUserCredentials($client_id, $username, $password) {
     ...
     *   The assertion.
     *
     * @return bool TRUE if the assertion is valid, and FALSE if it isn't. Moreover, if@code
     * return array(
     *   'scope' => <stored scope values (space-separated string)>,
     * );
     * @endcode
   ...
   * @ingroup oauth2_section_4
   */
  protected function setRefreshToken($oauth_token, $refresh_token, $client_id, $expires, $scope = NULL) {
    return;
  }
  ...
  protected function checkNoneAccess($client_id) {
     ...
     *
     * @return string A string that you want to send in a WWW-Authenticate header.@ingroup oauth2_error
     */
  ...
  public function verifyAccessToken($scope = NULL, $exitNotPresent = TRUE, $exitInvalid = TRUE, $exitExpired = TRUE, $exitScope = TRUE, $realm = NULL) {
    ...
    if (empty($token)){
      if($exitInvalid) {
      	$result = $this->errorWWWAuthenticateResponseHeader(OAUTH2_HTTP_VERIFICATION, $realm, OAUTH2_ERROR_INVALID_TOKEN, 'The access token provided is invalid.', NULL, $scope);
      	return $result;
      }else{
      	return FALSE;
      } 
...
//      return $exit_expired ? $this->errorWWWAuthenticateResponseHeader(OAUTH2_HTTP_UNAUTHORIZED, $realm, OAUTH2_ERROR_EXPIRED_TOKEN, 'The access token provided has expired.', NULL, $scope) : FALSE;
    ...
    if ($scope && (!isset($token["scope"]) || !$token["scope"] || !$this->checkScope($scope, $token["scope"]))) 
      return $exitScope ? $this->errorWWWAuthenticateResponseHeader(OAUTH2_HTTP_FORBIDDEN, $realm, OAUTH2_ERROR_INSUFFICIENT_SCOPE, 'The request requires higher privileges than provided by the access token.', NULL, $scope) : FALSE;

      //获取client_id的索引值
    $client = $this->getClient($token["client_id"]);
    $token["appId"] = $client["id"];
    if ($client['enabled'] == 0) {
        $this->errorWWWAuthenticateResponseHeader(OAUTH2_HTTP_LOCKED, NULL, SYSTEM_ERROR_APP_DISABLED, 'The app has been disabled.', NULL, NULL);
        return false;
    }
    return $token;
  }
     ...
     *
     *
     * @return string the value of sign,
     */
  private function sign($clientId, $clientSecret, $accessToken, $host, $port, $uri) {
      ...
      $indexHost = strpos($host, ":");
      if ($indexHost){
          $host = substr($host, 0, $indexHost);
      }
      //兼容https
      if($port=="443"){
          $port = "80";
    ...
    /**
     * sign the params, donnot sign port and http:// or https://
     *
     * @param $clientId
     *   Client identifier related to the access token.
     * @param $clientSecret
     ...
     *
     *
     * @return string the value of sign,@since 1.0.0
     */
  private function signWithout($clientId, $clientSecret, $accessToken, $uri) {
      // iis服务器，处理编码
      if (isset($_SERVER['SERVER_SOFTWARE']) && strpos(strtolower($_SERVER['SERVER_SOFTWARE']), 'iis') !== false)
      {
        // iis  urlencode() 或者 rawurlencode()，二者的区别是前者把空格编码为 '+'，而后者把空格编码为 '%20'
      ...
      $url = "{$uri}?access_token={$accessToken}&client_id={$clientId}&client_secret={$clientSecret}";
      return md5($url);
  }
     ...
     *
     * @return bool TRUE if everything in required scope is contained in available scope,@see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-5
     *
     * @ingroup oauth2_section_5
     */
  private function checkScope($required_scope, $available_scope) {
     ...
     *
     * @return bool Access token value if present, and FALSE if it isn't.@todo Support PUT or DELETE.
     *
...
//        $this->errorJsonResponse(OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_REQUEST, 'Malformed auth header');
  ...
  public function grantAccessToken() {
    ...
    //custom lifttime
    $this->setVariable("access_token_lifetime", 2592000); // 30day
    $this->setVariable("refresh_token_lifetime", 31536000); // 365day
   ...
   * 判断数据库中token是否存在
   */
  protected function getToken($client_id, $scope, $device_id){
      return false;
  }
  ...
  protected function getClient($client_id){
     ...
     * auth or POST data.
     *
     * @return array A list containing the client identifier and password, for example@code
     * return array(
     *   $_POST["client_id"],
     *   $_POST["client_secret"],
     * );
  ...
  protected function getClientCredentials() {
    if (isset($_SERVER["PHP_AUTH_USER"]) && $_REQUEST && isset($_REQUEST["client_id"]))
      $this->errorJsonResponse(OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_CLIENT);
    ...
    if ($_REQUEST && isset($_REQUEST["client_id"])) {
      if (isset($_REQUEST["client_secret"]))
        return array($_REQUEST["client_id"], $_REQUEST["client_secret"]);

      return array($_REQUEST["client_id"], NULL);
    }
  ...
  // End-user/client Authorization (Section 3 of IETF Draft).
     ...
     *
     * @return mixed The authorization parameters so the authorization server can prompt@see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-3
     *
     * @ingroup oauth2_section_3
     */
  ...
  private function doRedirectUriCallback($redirect_uri, $params) {
     ...
     *
     * @return string An absolute URI with supplied parameters.@ingroup oauth2_section_3
     */
  ...
  protected function createAccessToken($client_id, $scope = NULL) {
    ...
    $device_id = $this->getVariable('device_id');
    $this->setAccessToken($token["access_token"], $client_id, $device_id, time() + $this->getVariable('access_token_lifetime', OAUTH2_DEFAULT_ACCESS_TOKEN_LIFETIME), $scope);

    // Issue a refresh token also, if we support them
    if (in_array(OAUTH2_GRANT_TYPE_REFRESH_TOKEN, $this->getSupportedGrantTypes())) {
      $token["refresh_token"] = $this->genAccessToken();
      $this->setRefreshToken($token["access_token"], $token["refresh_token"], $client_id, time() + $this->getVariable('refresh_token_lifetime', OAUTH2_DEFAULT_REFRESH_TOKEN_LIFETIME), $scope);
      // If we've granted a new refresh token, expire the old one
      if ($this->getVariable('_old_refresh_token')){
          $this->unsetRefreshToken($this->getVariable('_old_refresh_token'));
      }
      if ($this->getVariable('_old_oauth_token')){
          $this->unsetOauthToken($this->getVariable('_old_oauth_token'));
      }
    }
  ...
  private function createAuthCode($client_id, $redirect_uri, $scope = NULL) {
     ...
     *
     * @return string An unique access token.@ingroup oauth2_section_4
     */
  protected function genAccessToken() {
     ...
     * other access token or auth code generation schemes.
     *
     * @return string An unique auth code.@ingroup oauth2_section_3
     */
  protected function genAuthCode() {
     ...
     *
     * @return bool The Authorization HTTP header, and FALSE if does not exist.@todo Handle Authorization HTTP header for non-Apache web servers.
     *
     * @ingroup oauth2_section_5
     */
