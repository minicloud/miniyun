	...
	public function getModulePaths()
	{
		if ($this->_modulePaths === null) {
			...
			foreach(Yii::app()->modules as $module => $config) {
				if (is_array($config)) {
					$alias = 'application.plugins.' . $module . '.migrations';
					...
					$this->_modulePaths[$config] = 'application.plugins.' . $config . '.migrations';
				}
			}
		}
	...
	public function actionMark($args)
	{
		...
		foreach($command->queryColumn() as $version) {
			...
			$this->getDbConnection()->createCommand()->update(
				$this->migrationTable,
				array('module' => $module, "created_at"=>date("Y-m-d H:i:s",time()), "updated_at"=>date("Y-m-d H:i:s",time())),
				'version=:version',
