    ...
    private  function newTable(){
        ...
        $this->createTable(DB_PREFIX.'_break_files',
            array(
                ...
                'uploaded_size'        => 'long NOT NULL',//已经上传的文件的大小
        ...
        //迷你云将具备下面能力
        //接口1：接受来自迷你存储服务器发送的断点文件报俊信息
        //接口2：接口来自迷你存储服务器冗余备份成功的报俊信息
        //任务1：定时检查存储服务器的状态
        //任务2：定时把冗余备份请求发送到存储服务器上
        //业务1：根据store_nodes.saved_file_count的值大小轮训分配服务器给文件上传接口，结果将输出到plugin_info中与秒传接口中
        //业务2：根据store_nodes.download_file_count的值大小轮训分配服务器给下载上传接口
        //业务3：图片请求缩略图，如迷你云本机服务器缓存没有，则像迷你存储请求，并计算该缩略图
        //业务4：文件上传成功，如果是图片，可下载文件到本地并提取图片的元数据，比较地理信息等（TODO暂时不做）
        
        //迷你存储将具备下面能力
        //接口1：接受来自客户端上传的断点文件，并将其写入本地磁盘。不用写缓存文件，直接写到硬盘中。
        //      写文件成功后，向迷你云报俊断点文件信息
        //接口2：接受来自客户端的下载请求(这里包括有业务1，使用Node计算，Nignx的sendfile模式文件下载)
        //接口3：接口来自迷你云的冗余备份请求，冗余备份成功后，向迷你云报俊冗余备份成功
