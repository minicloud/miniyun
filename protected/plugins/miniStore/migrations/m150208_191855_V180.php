    ...
    private  function newTable(){
        ...
        $this->createTable(DB_PREFIX.'_store_nodes',
            array(
                ...
                'safe_code'            => 'varchar(128) NOT NULL',//访问token，用于安全校验
        ...
        //迷你云将具备下面能力
        //接口1：客户端秒传接口，如是新建，为其随机分配迷你存储节点并将信息记录在在break_files，如还是上传该文件，且尚未上传成功，直接返回该迷你存储节点（11）
        ...
        //业务2：根据store_nodes.download_file_count的值大小轮训分配服务器给下载上传接口(11)
        //业务3：图片请求缩略图，如迷你云本机服务器缓存没有，则像迷你存储请求，并计算该缩略图(11)
        //业务4：文件上传成功，如果是图片，可下载文件到本地并提取图片的元数据，比如地理信息等（TODO暂时不做）
        
        //迷你存储将具备下面能力
        //接口1：接受来自客户端秒传接口，判断文件是否成功，如果本地有该文件内容，则直接上传成功。否则返回文件的断点信息即可（11）
        //接口2：接受来自客户端上传的断点文件，并将其写入本地磁盘。不用写缓存文件，直接写到硬盘中。（11）
        //接口3：接受来自客户端的下载请求(这里包括有业务1，使用Node计算，Nignx的sendfile模式文件下载)（11）
        //接口4：接口来自迷你云的冗余备份请求，冗余备份成功后，向迷你云报俊冗余备份成功
        //业务1：本地磁盘将有多个目录，这里有一个找文件存储目录的过程（11）
  }
}